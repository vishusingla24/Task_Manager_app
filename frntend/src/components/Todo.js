...
const Todo = ({ toast, todo, setTodo }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    await axios
      .post(`${API_BASE_URL}/todo/postTodo`, newTodoItem)
      .then((res) => {
        console.log(res);
      })
      .catch((err) => console.log(err));
  ...
  function tickTodo(todoId, status) {
    ...
    axios
      .patch(`${API_BASE_URL}/todo/updateTodo/${todoId}`, {
        status: status,
      })
      .then((res) => {
        console.log(res);
...
function deleteTodo(todoId) {
  axios
    .delete(`${API_BASE_URL}/todo/deleteTodo/${todoId}`)
    .then(() => {
